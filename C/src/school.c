...
char read_choice_line() {
    ...
    for (size_t i = 0; buffer[i] != '\0'; i++) {
        if (!isspace((unsigned char)buffer[i])) {
            return (char)toupper((unsigned char)buffer[i]);
        }
    }
...
void input_data() {
    ...
    delayed_print(buffer);
    new_student.grade = read_choice_line();
    ...
    if (found_count > 0) {
        ...
        if (option == 'E') {
            ...
            if (idx == -1) {
                ...
                char tempGrade = read_choice_line();
...
void show_menu() {
    ...
    while (1) {
        ...
        if (fgets(choice_str, MAX_FIELD_LENGTH, stdin) == NULL) {
            return;
        }
