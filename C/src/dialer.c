...
void play_sample(const char *sample_path) {
    ...
    // Human/audio mode is deliberately sequential: wait for this sample to
    // finish so it cannot overlap the next dialler sound.
    system(command);
}
...
#ifdef __APPLE__
    snprintf(command, sizeof(command),
             "/usr/bin/afplay -t 0.08 \"%s\" >/dev/null 2>&1",
             sample_path);
#else
    snprintf(command, sizeof(command),
             "timeout 0.08 aplay -q \"%s\" >/dev/null 2>&1",
             sample_path);
#endif

    // Each digit completes before the next begins. This keeps DTMF tones in
    // sequence and guarantees that the following phone/modem sample cannot
    // start while dialling is still audible.
    system(command);
}
...
void read_and_print_systems_found() {
    ...
    // On macOS/BSD, an a+ stream may begin with the read position at EOF.
    // Rewind before reading the saved systems.
    rewind(file);
    ...
    while(1) {
        ...
        if (index < 0 || index >= record_count) {
            ...
            if(strcmp(system_name[index],"system")==0) {
                play_sample("./samples/phone-busy-1.wav");
                clear_screen();
                fflush(stdout); // Flush the output buffer to ensure the prompt is displayed
                usleep(2000000);
            } else {
            ...
            // Discard any terminal input left behind by the connected system,
            // then remain in the discovered-systems view.
    ...
    for (int i = nd_start; i <= nd_end; i++) {
        ...
        if (hit == 'Y') {
            ...
            if (random_number == 1 && s1 == 0) {
                ...
                s1 = 1;
                play_sample("./samples/sauls-fish-market.wav");
            } else if (random_number == 2) {
                play_sample("./samples/phone-busy-1.wav"); 
            } else if (random_number == 3) {
                play_sample("./samples/hello.wav");
            } else if (random_number == 4) {
                play_sample("./samples/disconnected-2x.wav");
            } else if (random_number == 5 && s5 == 0) {
                s5 = 1;
                play_sample("./samples/funny-answer-phone.wav");
            } else {
                play_sample("./samples/phone-beep.wav");
            }
        ...
        if (hit == 'Y') {
            ...
            if (random_number == 1 && s1 == 0) {
                ...
                play_sample("./samples/telephone-ringing.wav");
